...
/**
 * creates a planar graph according to the method presented in the thesis. according to the method
 * presented, every graph is planar that has a maximum of 4 nodes with more than 2 neighbors.
 *
...
public final class Graph {
     ...
     * random node. Additionally further random edges can be drawn in.
     *
    ...
    private void connectUnconnectedNodes(final ArrayList<Node> nodes) {
        ...
        while (unconnectedNodes.size() > 0) {
            node1 = unconnectedNodes.get(0);
            // take a randomly connected node to make sure that the new node is connected to the
            // graph
            ...
            if (canConnect(node2)) {
                ...
                if (node2.getNeighbourCount() > MAX_NEIGHBOURS
                        && !nodesWithManyNeighbours.contains(node2))
    ...
    private void addMoreEdges(final int connections, final ArrayList<Node> nodes)
        ...
        for (int i = 0; i < connections; i++) {
            ...
            if (!node1.equals(node2) && canConnect(node1, node2)) {
                ...
                if (connect) {
                    ...
                    if (node1.getNeighbourCount() > MAX_NEIGHBOURS
                            && !nodesWithManyNeighbours.contains(node1))
    ...
    /**
     * if not: the selected node would have too many neighbors with the new connection, but is not
     * yet in the list but the list is full
     *
     * @param n
     * @return
     */
    private boolean canConnect(Node n) {
        return (!(n.getNeighbourCount() >= MAX_NEIGHBOURS
                && !nodesWithManyNeighbours.contains(n)
    ...
    private boolean canConnect(Node n1, Node n2) {
        ...
        boolean addN1 =
                (n1.getNeighbourCount() >= MAX_NEIGHBOURS && !nodesWithManyNeighbours.contains(n1));
        boolean addN2 =
                (n2.getNeighbourCount() >= MAX_NEIGHBOURS && !nodesWithManyNeighbours.contains(n2));
